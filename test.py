...
def message_processor(stop_event):
    global last_time, prices_data, cumulative_volume, cumulative_volumes, ATO, prices, models, mo_lo_changerate, metadata
    ...
    while not stop_event.is_set() or not message_queue.empty():
        try:
            ...
            if "LimitOrderBook" in message:
                ...
                if bid == None or len(bid) == 0: return 
                if offer == None or len(offer) == 0: return       
            ...
            # ===========================================================================================================        
                ...
                current_time = (time.hour, time.minute)
                if is_market_open(current_time) == False: return

                if current_time != last_time:
                    # for transforming the data structure of the last price in to a list of array
                    for s in prices: 
                        to_append = []
                        if len( prices[s][-1] ) > 0:
                    ...
                    for s in cumulative_volume:
                        ...
                        if time_cmp( start_time, current_time ) == -1 and time_cmp( end_time, current_time ) == 1:
                            ...
                            if len(cumulative_volumes[s] >= 2):
                                ...
                                if (s in models) and (current_time in models[s]) and (end_time in models[s][current_time]):
    ...
    #                           up_or_down = 0 # -> svm predict , up = 1 and don't go up or down = 0 and down = -1
                        ...
                        # At the end (current_time == end_time)
